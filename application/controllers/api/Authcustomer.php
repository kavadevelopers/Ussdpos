...
class Authcustomer extends CI_Controller
{
	...
	public function getprofile()
	{
		if($this->input->post('user')){
			$user = $this->customer_model->getCustomerData($this->input->post('user'));
			$reviews = $this->db->get_where('customer_review',['customer' => $this->input->post('user')])->result_array();
			$reviewsList = [];
	...
	{
		if($this->input->post('lat') && $this->input->post('lon') && $this->input->post('user') && $this->input->post('city') && $this->input->post('state') && $this->input->post('country') && $this->input->post('area') && $this->input->post('street')){	
			...
			if($customer['stripeclient'] == NULL || $customer['stripeclient'] == ""){
				$stpCustomer = $this->stripelib->createCustomer(
				...
				if($stpCustomer){
					$this->db->where('id',$this->input->post('user'))->update('customer',['stripeclient' => $stpCustomer->id]);
				}
			}else{
				$stpCustomer = true;
			}
			...
			if($stpCustomer){
				...
				retJson(['_return' => true,'msg' => 'Address Saved','data' => $this->customer_model->getCustomerData($this->input->post('user'))]);	
			}else{
				$strError = !empty($this->stripelib->api_error)? '('.$this->stripelib->api_error.')':'';
				retJson(['_return' => false,'msg' => 'Stripe Client Creation Failed. '.$strError]);	
			}
		}else{
			retJson(['_return' => false,'msg' => '`lat`,`lon`,`city`,`state`,`country`,`area`,`street` and `user`(userid) are Required']);
		}
	}
